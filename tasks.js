    ...
    function showTasks(filter = 'all', searchTerm = '') {
        ...
        if (selectedSubject && selectedLesson !== "" && window.tasksData[selectedSubject]) {
            ...
            lesson.tasks.forEach(task => {
                const status = tasksStatus[task.task] || 'pending';
                if ((filter === 'all' || filter === status) && 
                    ...
                    taskElement.innerHTML = `
                        ...
                        <button onclick="toggleTaskStatus('${task.task}')" class="mt-2 px-4 py-2 rounded-lg ${status === 'completed' ? 'bg-green-500' : 'bg-red-500'} text-white">
